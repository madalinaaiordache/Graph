...
instance Show a => Show (AlgebraicGraph a) where
    show (Node x) = show x
    show (Overlay x y) = let l1 = show x ++ "+" ++ show y in "(" ++ l1 ++ ")"
    show (Connect x y) = let l2 = show x ++ "*" ++ show y in "(" ++ l2 ++ ")"
...
mergeNodes :: (a -> Bool)       -- proprietatea îndeplinită de nodurile îmbinate
           ...
           -> AlgebraicGraph a  -- graful existent
           -> AlgebraicGraph a  -- graful obținut
mergeNodes prop node graph = do
    let cond x = if prop x then node else x
    fmap cond graph 
...
filterGraph prop graph = do
    let cond x = if prop x then Node x else Empty
    extend cond graph
{-
    *** TODO ***
...
removeNode node graph = let cond a = (a /= node) in filterGraph cond graph